...
Les sociétés généralement supprime les ordre de retour après leur facturation. Lorsque vous validez une facture, [!INCLUDE [prod_short](includes/prod_short.md)] transférée vers la page **Avoir achat enregistré**. Si vous avez activé la case à cocher **Expédition retour sur avoir** sur la page **Paramètres achats**, la facture est transférée vers la page **Expédition retour enregistrée**. Vous pouvez supprimer les documents à l’aide du traitement par lots **Supprimer les retours achat facturés**. Avant de supprimer les docs, le traitement par lots vérifie si les retours achat ont été entièrement livrés et facturés.  
...
Les sociétés suppriment Les commandes service facturées automatiquement après avoir été entièrement facturées. Lors de la validation d’une facture, une écriture correspondante est générée et peut être affichée sur la page **Factures service enreg.**.  
...
> [!NOTE]
> Après avoir exécuté la compression des dates, vous ne pouvez pas annuler les écritures comptables des fournisseurs ou des banques pour les transactions affectées par la compression.
