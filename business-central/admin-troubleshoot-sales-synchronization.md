...
description: 'Cet article offre des instructions pour identifier, résoudre les problèmes et les erreurs de synchronisation.'
author: brentholtorf
ms.topic: conceptual
ms.devlang: al
...
Les erreurs se produisent souvent à cause de quelque chose qu’un utilisateur a fait pour coupler des enregistrements ou parce que la configuration de l’intégration est fausse. Pour les erreurs liées aux enregistrements couplés, les utilisateurs peuvent les résoudre eux-mêmes. Les causes de ces erreurs sont souvent des actions telles que la suppression de données dans l’une des applications métier mais pas les deux, puis la synchronisation. Pour plus d’informations, voir [Afficher le statut d’une synchronisation](admin-how-to-view-synchronization-status.md).
