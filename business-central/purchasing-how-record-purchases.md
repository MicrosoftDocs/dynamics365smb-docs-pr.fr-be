...
Vous pouvez facilement corriger ou annuler une facture achat validée avant de payer le fournisseur. Par exemple, si vous souhaitez corriger une coquille, ou si vous souhaitez modifier l’achat assez tôt dans le processus de commande. En savoir plus, [Corriger ou annuler des factures achat impayées](purchasing-how-correct-cancel-unpaid-purchase-invoices.md) Pour contrepasser l’achat pour des articles ou services sur la facture achat validée pour lesquels le paiement est traité, créer un avoir achat. En savoir plus, [Traiter les retours ou annulations d’achats](purchasing-how-process-purchase-returns-cancellations.md)
...
3. Dans le champ **Qté à recevoir**, indiquez la quantité reçue.
   ...
   > [!NOTE]
   > Si la quantité reçue est supérieure à la quantité de la commande achat, et que le fournisseur a été configuré pour autoriser les sur-réceptions, utilisez le champ **Sur-réception** pour gérer la quantité excédentaire. Pour en savoir plus, consultez la section [Pour réceptionner plus d’articles que vous commandés](purchasing-how-record-purchases.md#receive-more-items-than-you-ordered).
...
1. Sélectionnez l’icône ![Ampoule qui ouvre la fenêtre de recherche](media/ui-search/search_small.png "Dites-moi ce que vous voulez faire"), entrez **Codes sur-réception.**, puis choisissez le lien associé.
