...
Le rapprochement bancaire est généralement un processus en plusieurs étapes. Tout d’abord, les relevés bancaires sont importés dans [!INCLUDE[prod_short](includes/prod_short.md)]. La correspondance des transactions avec les écritures comptables. Enfin, de nouvelles écritures comptables sont comptabilisées pour refléter toutes les transactions résiduelles qui n’étaient pas connues auparavant dans vos registres.

Copilot dans [!INCLUDE[prod_short](includes/prod_short.md)] réduit les efforts manuels en faisant correspondre davantage de transactions et en suggérant des comptes généraux dans lesquels vous pouvez effectuer une validation.
...
Nous recommandé aux utilisateurs de vérifier également l’exactitude et de corriger toute divergence après avoir quitté la fenêtre Copilot. Ce processus devrait inclure les activités suivantes :
