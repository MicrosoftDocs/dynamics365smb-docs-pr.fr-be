...
- Copilot convertit l’entrée en langage naturel en une requête composée de critères de recherche, de tri et de filtrage dans une table.
  ...
  La capacité utilise le [!INCLUDE[prod_short](includes/prod_short.md)] Les capacités natives de recherche de données de permettent de trouver des données correspondantes à partir de tables au sein de la base de données des entreprises. La recherche s’exécute sous votre propre identité pour des raisons de sécurité et de conformité. Il ne recherche pas en dehors du [!INCLUDE[prod_short](includes/prod_short.md)] base de données.
