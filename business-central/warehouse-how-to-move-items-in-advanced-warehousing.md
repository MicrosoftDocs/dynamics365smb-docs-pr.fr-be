...
3. Si nécessaire, saisissez votre ID utilisateur lorsque vous commencez à travailler sur un rangement.  
  ...
  > [!NOTE]  
  > Les lignes sont triées par ordre croissant, en fonction des critères sélectionnés. Si vous triez par document, le tri est effectué d’abord par type de document en fonction enum **Document source d’activité d’entrepôt** . Si vous triez par de livraison, le tri est effectué par type de destination en fonction du champ **Type de destination d’entrepôt** .
